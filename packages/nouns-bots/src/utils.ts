...
export function formatAuctionStartedTweetText(auctionId: number) {
 ...
 Learn more at https://nunouns.wtf`;
}
...
export function getAuctionEndingSoonTweetText() {
  return `This auction is ending soon! Bid now at https://nunouns.wtf`;
}

export function formatNewGovernanceProposalText(proposal: Proposal) {
  return `A new nuNounsDAO proposal (#${proposal.id}) has been created: ${extractProposalTitle(
    proposal,
  )}`;
}

export function formatUpdatedGovernanceProposalStatusText(proposal: Proposal) {
  return `nuNouns DAO proposal #${proposal.id} (${extractProposalTitle(
    proposal,
