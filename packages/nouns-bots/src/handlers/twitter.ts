...
import {
  ...
  getAuctionEnding10minTweetText,
...
export class TwitterAuctionLifecycleHandler implements IAuctionLifecycleHandler {
  ...
  async handleAuctionEnding10min(auctionId: number) {
    const tweetReplyId = await getAuctionReplyTweetId();

    try {
      if (!tweetReplyId) {
        console.error(`handleAuctionEnding10nin no reply tweet id exists for auction ${auctionId}`);
        return;
      }
      const tweet = await twitter.v1.reply(getAuctionEnding10minTweetText(), tweetReplyId);
      await updateAuctionReplyTweetId(tweet.id_str);
      console.log(`processed twitter auction 10 min to the end for auction ${auctionId}`);
    } catch {
