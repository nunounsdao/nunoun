...
export const isNouncubatorNoun = (nounId: BigNumber) => {
  return nounId.mod(10).eq(1) || nounId.eq(1);
};

const emptyNouncubatorAuction = (onDisplayAuctionId: number): Auction => {
  return {
 ...
 */
export const generateEmptyNouncubatorAuction = (
  nounId: BigNumber,
  pastAuctions: AuctionState[],
): Auction => {
  const nouncubatorAuction = emptyNouncubatorAuction(nounId.toNumber());
  ...
  if (auctionAboveStartTime) nouncubatorAuction.startTime = auctionAboveStartTime.toJSON();

  return nouncubatorAuction;
};
